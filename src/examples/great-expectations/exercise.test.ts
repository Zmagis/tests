...
import { defaultStatuses, KanbanBoard } from '$lib/kanban-board';
...
describe('Kanban Board', () => {
  it('should include "Backlog" in board.statuses', () => {
    ...
    expect.hasAssertions();
    expect(board).toBeInstanceOf(KanbanBoard);
  ...
  it('should remove a status using #removeStatus', () => {
    const board = new KanbanBoard('Things to Do');
    expect.hasAssertions();
    const oldStatus = defaultStatuses[0];
    // Use board.removeStatus to remove a status.
    expect(board.statuses).toContain(oldStatus);
    board.removeStatus(oldStatus);
    ...
    expect(board.statuses).not.toContain(oldStatus);
  });
});
